...
async def _get_vector_context(
    ...
    try:
        ...
        if query_param.allowed_chunk_ids is not None:
            results = [r for r in results if r.get("id") in query_param.allowed_chunk_ids]
...
async def _get_node_data(
    ...
    if query_param.allowed_chunk_ids is not None and results:
        results = [
            r for r in results
            if r.get("source_id") and any(
                c in query_param.allowed_chunk_ids
                for c in r["source_id"].split(GRAPH_FIELD_SEP)
            )
        ]
...
async def _find_related_text_unit_from_entities(
    ...
    for entity in node_datas:
        if entity.get("source_id"):
            ...
            if query_param.allowed_chunk_ids is not None:
                chunks = [c for c in chunks if c in query_param.allowed_chunk_ids]
            if chunks:
